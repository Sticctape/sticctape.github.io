...
async function verifyJWT(token, env) {
    ...
    const toBytes = (b64url) => Uint8Array.from(atob(b64url.replace(/-/g, '+').replace(/_/g, '/').padEnd(Math.ceil(b64url.length / 4) * 4, '=')), c => c.charCodeAt(0));
    ...
    if (env.JWT_AUD && payload.aud !== env.JWT_AUD)
        throw new Error('Bad aud');
    if (env.JWT_ISS && payload.iss !== env.JWT_ISS)
        throw new Error('Bad iss');
    if (!payload.sub)
        throw new Error('Missing sub');
    return payload.sub;
}
async function getOwnerId(req, env) {
...
async function isValidStaffToken(req, env) {
    const auth = req.headers.get('authorization');
    if (!auth || !auth.toLowerCase().startsWith('bearer '))
        return false;
    const token = auth.slice(7).trim();
...
async function handleListBottles(request, env, ownerId) {
...
async function handleCreateBottle(request, env, ownerId) {
    const body = await request.json();
    const id = crypto.randomUUID();
    const { brand, product_name, base_spirit, style, abv, volume_ml, quantity = 1, status = 'sealed', purchase_date, price_cents, currency = 'USD', location, notes, image_url, upc, tags } = body || {};
    ...
    try {
      ...
      purchase_date, price_cents, currency, location, notes, image_url, upc
            ...
            .bind(id, normalizedOwnerId, brand, product_name, base, styleVal, abvVal, volVal, quantity, statusVal, purchaseVal, priceVal, currencyVal, locationVal, notesVal, imageVal, upcVal);
        await stmt.run();
    }
    catch (err) {
...
async function handleUpdateBottle(request, env, id, ownerId) {
    const body = await request.json();
    // Verify ownership
    ...
    if (!(ownerId && existing.owner_id && ((ownerId.startsWith('owner:') && String(existing.owner_id).startsWith('owner:')) ||
    ...
    if (Array.isArray(body.tags)) {
      ...
      JOIN bottle_tags bt ON bt.tag_id = t.id WHERE bt.bottle_id = ?`).bind(id).all();
        ...
        // Remove extra
        for (const { id: tagId, name } of (current.results || [])) {
...
async function handleDeleteBottle(request, env, id, ownerId) {
    const existing = await env.DB.prepare(`SELECT owner_id FROM bottles WHERE id = ?`).bind(id).first();
    if (!existing)
        return json({ error: 'not found' }, { status: 404 });
    // Allow if both are owner-prefixed (normalized) or exact match otherwise
...
async function handleImageUpload(request, env, bottleId, ownerId) {
    // Verify bottle ownership first
    const bottle = await env.DB.prepare(`SELECT owner_id FROM bottles WHERE id = ?`).bind(bottleId).first();
    if (!bottle)
        return json({ error: 'not found' }, { status: 404 });
    if (!(ownerId && bottle.owner_id && ((ownerId.startsWith('owner:') && String(bottle.owner_id).startsWith('owner:')) ||
...
async function handleImageDelete(request, env, bottleId, ownerId) {
    // Verify bottle ownership first
...
async function handleUPCLookup(request, env, upc) {
...
export default {
    async fetch(request, env) {
        ...
        try {
            ...
            const imageUploadMatch = path.match(/\/api\/bottles\/(.+)\/image$/);
            if (imageUploadMatch && request.method === 'POST') {
                if (!isAdmin)
                    return withCORS(json({ error: 'not found' }, { status: 404 }), request);
                if (!ownerId)
                    return withCORS(json({ error: 'Unauthorized' }, { status: 401 }), request);
                return withCORS(await handleImageUpload(request, env, imageUploadMatch[1], ownerId), request);
            }
            // Image delete endpoint: DELETE /api/admin/bottles/{id}/image
            if (imageUploadMatch && request.method === 'DELETE') {
                if (!isAdmin)
                    return withCORS(json({ error: 'not found' }, { status: 404 }), request);
                if (!ownerId)
                    return withCORS(json({ error: 'Unauthorized' }, { status: 401 }), request);
                return withCORS(await handleImageDelete(request, env, imageUploadMatch[1], ownerId), request);
            }
